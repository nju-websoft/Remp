...
def attribute_alignment(attributes_1, attributes_2, prior_alignment, similarity_func=None):
    ...
    # attr_jaccard['weight'] = attr_jaccard['sum'] / attr_jaccard['size']
    g = nx.from_pandas_edgelist(attr_jaccard[attr_jaccard['weight'] >= 0.05], source='a1', target='a2', edge_attr=['weight'])
...
def attribute_alignment_slow(attributes_1, attributes_2, prior_alignment, similarity_func=similarity_func_default):
    (s, a, v) = ('s', 'a', 'v')
    # normalization
    ...
    for a1, A1 in groups1:
        for a2, A2 in groups2:
            ...
            paired = pd.merge(paired, A1[['s', 'v']].rename(columns={'s': 's1', 'v': 'v1'}), how='left')
            paired = pd.merge(paired, A2[['s', 'v']].rename(columns={'s': 's2', 'v': 'v2'}), how='left')
            total_cnt = np.sum(paired['v1'].notna() | paired['v2'].notna())
            paired = paired.dropna()
            if len(paired) > 0:
                score = paired.apply(similarity_func_default, axis=1).sum() / total_cnt
                if score >= 0.05:
